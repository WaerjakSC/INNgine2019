...
void HierarchyView::dragEnterEvent(QDragEnterEvent *event) {
    ...
    Entity *entity = Registry::instance()->getEntity(item->data().toUInt()); // One further cast to get the Entity, an overloaded QStandardItem that also contains an entity ID.
    if (entity)
